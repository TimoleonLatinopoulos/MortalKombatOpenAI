...
class Keras_DDQN:
    ...
    def conv_layer(_inputs, _filters, _kernel_size, _strides, _name):
        return Conv2D(filters=_filters, kernel_size=_kernel_size, strides=_strides,
                      ...
                      padding="valid", activation=tf.nn.relu, use_bias=False, name=_name)(_inputs)

    @staticmethod
    def dense_layer(_inputs, _units, _name):
        return Dense(units=_units, kernel_initializer=tf.variance_scaling_initializer(scale=2), name=_name)(_inputs)
